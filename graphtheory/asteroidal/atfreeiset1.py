...
class ATFreeIndependentSet:
    ...
    def __init__(self, graph):
        ...
        # cl[u] daje liste komponentow G\N[u] jako 2-tuple (reprezentanci).
        # Zbior mozna odczytac przez cs[a,b].
        self.intervals = dict()
        self.bucket_C = None
        self.bucket_I = None
    ...
    def find_alpha_G(self):
        ...
        results = []   # results for different vertices
        for v in self.graph.iternodes():
            s = sum(self.alpha_C[key] for key in self.component_lists[v])
            results.append(s)
        # Do wynikow z komponentow trzeba dorzucic v lezacy w N[v].
