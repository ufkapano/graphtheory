...
class TestNodeCover1(unittest.TestCase):
    ...
    def test_chordal_node_cover(self):
        algorithm = ChordalNodeCover(self.G, self.T)
        algorithm.run()
        expected = set(["B", "C", "G", "E"])
        self.assertEqual(algorithm.cardinality, len(expected))
        self.assertEqual(algorithm.node_cover, expected)
        # Testing node cover.
